...
<nav class='Mobile'>
    ...
    <div class="ScrollUsers">
        <h2 class='Title'><i class="fas fa-user-nurse"></i>&nbsppolecanydoktor.pl</h2>

        <!-- foreach -->
        @foreach($users as $user)
        <div class="UserCreateAdminContent">
            <div>
                <h2>{{$user->Email}}</h2>
            ...
            <div>
                <button><a href='/dashboard/admin/makeadminprocess/{{ $user->Id }}'>Zrób Adminem</a></button>
            </div>
        </div>
        @endforeach
    </div>
