...
<head>
    ...
    <title>Zarejestruj wizytę</title>
...
<nav class='Mobile'>
    ...
    <div class="Banner">
        <div class="RegisterChoose RegisterVisitForm">
            <h1>Rezerwacja wizyty u dr {{ $info->First_name }} {{ $info->Last_name }}</h1>
            <!-- <h2>Wybierz termin:</h2> -->
            <form action="/home/doctor/registervisitProcess/{{ $info->Id }}" style='text-align:center' method='post'>
                @csrf
                <h2>Wybierz termin:</h2>
                <select name="visitDatetime">
                    @foreach($info->dates as $date)
                    <option value="{{ $date }}">{{ $date }}</option>
                    @endforeach
                </select>
                <input type="submit" value='Wybieram ten termin'>
            </form>
        </div>
    </div>
